...
_event_handlers = [{"rising": nop, "falling": nop} for _ in range(8)]
...
def on(state, pin):
    def inner(func):
        ...
        if not state in valid:
            raise TypeError(
                f"{state} is not a valid pin state, valid states are: {', '.join(valid)}"
            )
        if pin >= 8 or pin <= 0:
            raise IndexError(
                f"{pin} is not a valid input, valid inputs are: {', '.join(range(8))}"
            )
